...
def calculate_swimability(water_temp, air_temp, wind_chill, humidity, wind_speed):
    """
    Computes a 0-100 Swimability Score based on human tolerance.
    Heavy emphasis on exit shock (wind chill + water temp combo).
    """
    # Water Temp component (40% weight) - Optimal is 72°F+ for Lake Michigan
    ...
    else: water_score = 10  # Freezing upwelling water
    
    # Thermal comfort component (40% weight) - Uses Wind Chill/Apparent Temp
    ...
    # Humidity & Wind Penalties (20% weight)
    # High wind makes exiting the water miserable
...
def estimate_fly_risk(wind_speed_mph, wind_dir_deg):
    """
    Estimates Biting Black Fly risk.
    Flies hatch in inland brush/woods and plague the beach if:
    1. Winds are gentle/light (they can fly easily).
    2. Wind blows OFFSHORE (West/Northwest), carrying them from woods to sand.
    """
    # Offshore wind slice for this specific North-South coastline line
    ...
    elif 60 <= wind_dir_deg <= 160: # Pure onshore lake breeze
...
try:
    ...
    # Title Banner
    st.title("🏖️ Kohler-Andrae Beach Monitor")
    st.caption(f"Real-time hydrodynamic & atmospheric forecast modeling for coordinates: {LAT}, {LON}. Updated: {datetime.now().strftime('%H:%M CDST')}")
    st.markdown("---")

    # Row 1: The Primary Custom Indexes
    ...
    with col8:
        st.metric("Wind Speed & Heading", f"{wind_speed:.1f} mph", f"{wind_dir}° Vector")
    with col9:
